  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      if (this.classList.contains('active-style')) {
        ...
        this.nextElementSibling.style.maxHeight = "".concat(this.nextElementSibling.scrollHeight + 80, "px");
      } else {
...
var calc = function calc(sizeSelector, materialSelector, optionsSelector, promocodeSelector, resultSelector) {
  ...
  var calcSum = function calcSum() {
    ...
    var sum = Math.round(+sizeBlock.value * +materialBlock.value + +optionsBlock.value);
...
var filter = function filter() {
  ...
  var typeFilter = function typeFilter(cards) {
  ...
  menu.addEventListener('click', function (e) {
    ...
    if (target && target.tagName === 'LI') {
      ...
      var menuItemClass = target.classList[0];
      target.classList.add('active');
      var cards = wrapper.querySelectorAll(".".concat(menuItemClass));
      typeFilter(cards);
    }
  });
};
...
var forms = function forms() {
  ...
  var send = function send(url, formData) {
    var status, statusImg, statusText;
    return regeneratorRuntime.async(function send$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            status = document.querySelector('.status');
            statusImg = status.childNodes[0], statusText = status.childNodes[1];
            statusImg.setAttribute('src', message.spinner);
            statusText.textContent = message.loading;
            _context.next = 6;
            return regeneratorRuntime.awrap(Object(_services_requests__WEBPACK_IMPORTED_MODULE_14__["postData"])(url, formData));
  ...
  uploads.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var finalName = name.substring(0, 6) + dots + extension;
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var api = item.closest('.popup-design') || item.classList.contains('form-calc') ? path.designer : path.question;
      console.log(api);
      var formData = new FormData(item);
      send(api, formData).then(function (res) {
        console.log(res);
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getData"])('assets/db.json').then(function (data) {
...
var sliders = function sliders(itemsSelector, direction, prevBtnSelector, nextBtnSelector) {
...
var postData = function postData(url, formData) {
  var response;
